...
class Obst(Task):
    ...
    def propose_prompt_wrap(x: str, y: str='') -> str:
        formal_input = "k=" + str(x[0][0]) + ",freq=" + str(x[0][1])
        prompt = propose_prompt.format(input=formal_input) + y
        print(prompt)
    ...
    def value_outputs_unwrap(x: str, y: str, value_outputs: list) -> float:
        ...
        value = int(re.findall(r'\d', value_outputs[-1])[-1]) if value_names else None
...
#cost_value, r_values, cost, tree_array = optimalSearchTree([1,2,3,4,5], [3,7,11,15,19], 5)
